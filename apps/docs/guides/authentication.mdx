...
`btca wipe` is a destructive full reset that removes local BTCA config/data and OpenCode auth
credentials for all providers (with a confirmation prompt unless `--yes` is passed).
