...
Permanently deletes all local BTCA state.
...
- Removes global BTCA config/data directories.
- Removes local provider auth storage.
- Scans your home directory for BTCA project configs and `.btca` data folders and deletes them.
- Prints removed, skipped, and failed paths.
- Refuses dangerous targets (for example root, home, or project-root ancestor paths).
